...
lc_change = raster("~/UpennMUSA/Semester_2/CPLN_675/Urban_Growth_ATX/Austin_Data/urbn_grwth_08_19.tif")

#counties that define Austin MSA
...
# Reclassification happened in arcGIS
...
fishnet <- fishnet_join%>%
  mutate(lc_change = ifelse(urbn_grwth_08_19 == 0,0,1),
...
![Land Cover 2008](~/UpennMUSA/Semester_2/CPLN_675/Urban_Growth_ATX/Graphics/LC_08.png)

```{r, warning = FALSE, message = FALSE}

fishnet_fid<- st_read('~/UpennMUSA/Semester_2/CPLN_675/Urban_Growth_ATX/Austin_Data/Fishnet.shp')
fishnet <- cbind(fishnet,fishnet_fid%>%st_drop_geometry)%>%
  dplyr::select(-urbn_grwth_08_19)%>%
  rename(FID = FID_1)

#Read LC_08 in, rename majority landcover type
LC_08<- read.csv('~/UpennMUSA/Semester_2/CPLN_675/Urban_Growth_ATX/Austin_Data/LC_08.csv')%>%
  dplyr::select(FID, MAJORITY)%>%
  rename(LC_08 = MAJORITY)

#Read LC_08 in, rename majority landcover type
LC_19<- read.csv('~/UpennMUSA/Semester_2/CPLN_675/Urban_Growth_ATX/Austin_Data/LC_19.csv')%>%
  dplyr::select(FID, MAJORITY)%>%
  rename(LC_19 = MAJORITY)

LC_08 <- fishnet%>%left_join(., LC_08, by = 'FID')
...
# ggplot() +
#   geom_sf(data=austinMSA) +
#   geom_raster(data=rast(test) %>% na.omit %>% filter(value > 0), 
#               aes(x,y,fill=as.factor(value))) +
